...
//        }}</ul>
    ...
    for(m <- urlRegex.findAllIn(text).matchData) {
      ...
      sbProcessedRes.append(url match {
        ...
        case u if u.startsWith("http") => """<a href="%s">%s</a>""".format(u,u)
        case md5 => """ <a href="%sservices/jobs/list.html?jobID=%s">%s</a> [<a href="%sservices/jobs/console.txt?jobID=%s">Console</a>,<a href="%sservices/jobs/log.txt?jobID=%s">Log</a>,<a href="%sservices/jobs/kill.html?jobID=%s" onclick="return confirm('Are you sure you want to aborth job %s?');">Kill</a>] """ format (prefix,md5.trim,md5.trim,prefix,md5.trim,prefix,md5.trim,prefix,md5.trim,md5.trim)
      })
