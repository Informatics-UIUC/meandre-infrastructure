 ...
 * therefore returns all files with depth 'n' before returning those with 
 * depth 'n+1'.
 *
 * <p>Only returns regular files: will never give a File representing a
 * directory as a result from the next() method.
 *
 * <p>This class is itself an iterator over files (Iterator<File>). To use
 * it:
 * <p>
 * Iterator<File> myFileIter = new FileTreeIterator(myDirectory);<br>
 * while(myFileIter.hasNext(){<br>
 *     File next = myFileIter.next();</br>
 *     //process 'next'</br>
 *     }
 *  
 *
...
public class FileTreeIterator implements Iterator<File>{
     ...
     * holds the next value to return if it exists, or null if there are
     * no more files to return.
     */
    File _lookahead;
    
    /**
     * create an iterator over all files in the subdirectory of
     * the input directory treeDir.
     */
    public FileTreeIterator(File treeDir){
    ...
    /**
     * for the input directory, add any regular files (not directories) 
     * to this class instance's _currentDirContents.
     */
