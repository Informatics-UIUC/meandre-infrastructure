...
extends Thread {
	 ...
	 */
	public void probeInitializeWrappedComponent ( WrappedComponent wc ) {
		WrappedComponent wcc = wc;
		
		// Check for state serialization
		if ( bStateSerialization ) 
			wcc = serializeState(wc);
		
		Object[] oa = {Probe.ProbeCommands.EXECUTABLE_COMPONENT_INITIALIZED,wc.getExecutableComponentInstanceID(),wcc,new Date()};
		clqStatements.add(oa);
		semWorkAvailable.release();
	}
	 ...
	 * @param wc The wrapped component
	 */
	public void probeDisposeWrappedComponent ( WrappedComponent wc ) {
		WrappedComponent wcc = wc;
		...
		Object[] oa = {Probe.ProbeCommands.EXECUTABLE_COMPONENT_DISPOSED,wc.getExecutableComponentInstanceID(),wcc,new Date()};
		clqStatements.add(oa);
		semWorkAvailable.release();
	}
	 ...
	 */
	protected WrappedComponent serializeState ( WrappedComponent wc ) {
		log.warning("State serialization not implemented. State serialization discarded");
		// TODO: Implement the state serialization
		return (WrappedComponent) wc;
	}
