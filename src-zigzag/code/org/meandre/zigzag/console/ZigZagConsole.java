...
public class ZigZagConsole {
    ...
    private void showFlow(String[] saLine) {
        if ( saLine.length!=2 || !saLine[1].equals("flow") ) {
            ...
            Runnable run = new Runnable() {
                @Override
                public void run() {
                    FlowDrawer.fireViz(fg.getCurrentFlowDescription("http://seasr.org/zigzag", true));
                }
            };
    ...
    private void saveFile(String[] saLine) {
        if ( saLine.length<3 || saLine.length > 4 || (saLine.length == 3 && !saLine[1].equals("zigzag"))) {
            System.out.println();
            ...
            if ( sFormat.equals("zigzag") ) {
                ...
                try {
                    ...
                    System.out.println();
                    System.out.println("\t File "+sFileName+" could not be saved.");
                    System.out.println();
                } catch (IOException e) {
                    System.out.println();
                    System.out.println("\t Problem while closing file "+sFileName+".");
                    System.out.println();
                }
            }
            else if ( sFormat.equals("mau") ) {
                String sBaseURL = saLine[3];

                // Save the MAU file
                try {
                    fg.generateMAU(sFileName, sBaseURL);
                } catch (ParseException e) {
                    System.out.println();
                    System.out.println("\t File "+sFileName+" could not be saved.");
                    System.out.println();
                ...
                try {
                    ...
                    FlowDescription fd = fg.getFlowDescription(sFileName, sBaseURL, true);
    ...
    private void listCommand(String[] saLine) {
        ...
        if ( saLine.length==2 && bError ) {
            ...
            if ( fg.getInstances().keySet().contains(sInsName) ) {
                ...
                FlowDescription fd = fg.getCurrentFlowDescription("http://seasr.org/zigzag", false);
    ...
    private void printHelp(String[] saLine) {
        ...
        if ( saLine.length==1 ) {
            ...
            System.out.println("\t save [zigzag|mau|rdf|ttl|nt] <file_name> [baseURL]: ");
            ...
            System.out.println("\t                  The flow is saved as a MAU file if requested. Also the RDF version ");
            System.out.println("\t                  of the flow can be saved in RDF/XML, TTL, and NT dialects.");
            System.out.println("\t                  The baseURL is required if saving as MAU or as any of the RDF dialects");
        }
        else if ( sCmd.equals("show") ) {
