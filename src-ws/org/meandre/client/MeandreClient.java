...
//////////
     ...
     */
    public Set<URL> retrieveLocations() throws TransmissionException{
        ...
        String jElementKey = "location";
        Set<String> ssLocs = unpackJSONArray(jtLocs, jArrayKey, jElementKey);
        Set<URL> locUrls = convertStringSetToUrls(ssLocs);
        return locUrls;
    }
...
///////////
     ...
     */
/*
 *     TODO: this would require streaming the results back. need to change to
 *     separate startFlowRun() and pollFlowRun() methods.
 *     
       public String runFlow(String sFlowUrl, boolean verbose) 
            throws TransmissionException{
        String sRestCommand = "services/execute/flow.txt";
        sRestCommand = appendHttpParam(sRestCommand, "uri", sFlowUrl);
        if(verbose){
            sRestCommand = appendHttpParam(sRestCommand, "statistic", "true");
        }else{
            sRestCommand = appendHttpParam(sRestCommand, "statistic", "false");
        }
        String sResults = executeGetRequest(sRestCommand);
        return sResults;
    }
*/
     ...
     *
     * @return a map where the keys are flow id urls, and the values are 
     *
     *<p> calls:
